"""
Package: Catálogo de colores. SPRTMNTRPP-82.
Un endpoint /api/catalogs/color con GET (list), POST/PUT/DELETE masivo.
"""
...
from .create_color import handle_create
from .delete_color import handle_delete
from .list_color import handle_list
from .update_color import handle_update
...
def catalog_color(req: https_fn.Request) -> https_fn.Response:
    ...
    validation_response = validate_request(
        req, ["GET", "POST", "PUT", "DELETE"], "catalog_color", return_json_error=False
    )
    if validation_response is not None:
        return validation_response

    try:
        if not verify_bearer_token(req, "catalog_color"):
