...
@patch("users.user_route.verify_bearer_token", return_value=True)
@patch("users.user_route.validate_request", return_value=None)
@patch("users.user_route.update_my_route_notes_handle")
def test_user_route_put_my_route_notes_dispatches_to_update_notes(
    mock_update_notes, mock_validate, mock_verify
):
    from users.user_route import user_route

    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_update_notes.return_value = mock_resp
